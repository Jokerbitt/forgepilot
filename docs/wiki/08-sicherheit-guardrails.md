...
- **Policy-Gate vor dem Start** (`src/lib/policy/gate.ts`): Vor jedem Agenten-Start prüft die Deny-first-Policy den Auftrag (Risk-C, Secret-/destruktive Tools, fehlendes Budget, öffentlicher Privacy-Modus). **Scharf** (Standard in der Produktion, `FORGEPILOT_POLICY_ENFORCE=1`): ein „deny" stoppt den Start hart (HTTP 403). Mit `FORGEPILOT_POLICY_ENFORCE=0` läuft er als Report-Only (protokolliert nur).
  **Menschliche Freigabe zählt:** Zwei der Regeln (`risk-class-c`, `privacy-public`) fordern *„menschliche Freigabe erforderlich"* — sie sind mit einer echten Freigabe **erfüllt** und blockieren dann nicht mehr. Sonst würde das scharfe Gate ausgerechnet die Läufe abweisen, die du gerade über den ADR-004-Pfad freigegeben hast. Was im Protokoll durch die Freigabe abgedeckt wurde, wird als Eintrag festgehalten. **Absolut** bleiben Secret-Zugriff, destruktive Tools, fehlendes Budget und leeres Ziel — die kann keine Freigabe aushebeln.
...
- **Smoke-Gate vor dem Writeback** (`src/lib/agent-runner/runtime-bootstrap.ts`, `FORGEPILOT_SMOKE_GATE=1`): Build- und Test-Gate beweisen nur, dass der Code **kompiliert und die Tests bestehen** — nicht, dass die App wirklich läuft (ein RSC-/Laufzeitfehler kommt durch beide durch). Scharf geschaltet startet der Runner die App deshalb **aus dem Arbeitsbereich des Agenten**, ruft die Kernrouten ab (`/`, `/login`, `/dashboard`, `/api/health`) und scannt das Server-Log auf Absturzsignaturen — **bevor** das Ergebnis ins Ziel-Repo übernommen wird. Rot = Delegation schlägt fehl, nichts wird übernommen. Ohne Flag läuft der Smoke-Test wie bisher erst **nach** der Übernahme als reine Diagnose-Zeile.
...
> Hinweis: Der Policy-Gate ist inzwischen **scharf** (`FORGEPILOT_POLICY_ENFORCE=1` ist der Produktions-Default in `docker-compose.yml`). Report-Only erreichst du mit `FORGEPILOT_POLICY_ENFORCE=0`. Details in `docs/adr/ADR-003-runner-autonomy-security.md`.
