...
export interface ResearchStats {
  ...
  academicRatio: number          // 0-1 across all completed docs
  totalTokens: number
...
export function computeResearchStats(): ResearchStats {
  ...
  const citationsByCredibility: Record<SourceCredibility, number> = {
    academic: 0, government: 0, reputable: 0, general: 0, unknown: 0,
  }
  ...
  for (const doc of completed) {
    for (const c of doc.citations) {
      totalCitations++
      citationsByCredibility[c.credibility] = (citationsByCredibility[c.credibility] ?? 0) + 1
    }
    if (doc.tokenUsage) {
      totalTokens += doc.tokenUsage.promptTokens + doc.tokenUsage.completionTokens
    }
  }
  ...
  return {
    ...
    totalCitations,
    academicCitations: citationsByCredibility.academic,
    ...
    academicRatio: totalCitations > 0
      ? Math.round(((citationsByCredibility.academic + citationsByCredibility.government) / totalCitations) * 100) / 100
      : 0,
    totalTokens,
    topTags,
  }
