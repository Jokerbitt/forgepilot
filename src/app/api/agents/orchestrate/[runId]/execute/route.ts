    ...
    try {
      ...
      const delegationPayload = {
        ...
        contract: {
          ...
          context: `Part of orchestrated run ${runId}. Skill category: ${task.skillCategory}. File patterns: ${task.filePatterns.join(', ')}`,
          branchStrategy: `feature/orch-${runId.slice(-6)}-${task.id.slice(-6)}`,
          requiresApproval: false,
          privacyMode: 'private-cloud',
          outputMode: 'text',
          riskClass: 'A',
          definitionOfDone: task.acceptanceCriteria.join('\n'),
        },
