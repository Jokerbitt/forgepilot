...
export async function GET() {
  ...
  const projects: ProjectSummary[] = briefs.map((brief: ProjectBrief) => {
    ...
    const planningMode = brief.planningMode ?? 'beginner'
    ...
    const platformGuidance = brief.platformGuidance ?? platformPromptGuidance(targetPlatform, brief.customPlatformNote)
    const persistenceGuidance = brief.persistenceGuidance ?? persistencePromptGuidance(persistenceStrategy)
...
function buildPlanningSummary(brief: ProjectBrief, delegations: Delegation[]) {
  ...
  const activeAgents = linkedDelegations
  ...
  const recentDelegations = [...linkedDelegations]
    ...
    .slice(0, 6)
    .map(toDelegationSummary)
  const pmPlan = buildProjectManagerPlan({
    briefId: brief.id,
    workPackages,
    linkedDelegations,
    failedDelegations,
  ...
  if (failedDelegations > 0 || blockedWorkPackages > 0) {
    return { status: 'attention' as const, metrics, progress, activeAgents, recentDelegations, pmPlan, nextAction: { label: 'Blocker klaeren', href: `/delegations?briefId=${brief.id}&status=failed`, tone: 'danger' as const } }
  }
  if (workPackages.length > 0 && completedWorkPackages === workPackages.length) {
    return { status: 'completed' as const, metrics, progress, activeAgents, recentDelegations, pmPlan, nextAction: { label: 'Abschluss pruefen', href: `/project-briefs/${brief.id}`, tone: 'success' as const } }
  }
  if (runningDelegations > 0 || linkedDelegations.some(delegation => delegation.status === 'approved' || delegation.status === 'pending')) {
    return { status: 'in_progress' as const, metrics, progress, activeAgents, recentDelegations, pmPlan, nextAction: { label: 'Delegationen steuern', href: `/delegations?briefId=${brief.id}`, tone: 'info' as const } }
  }
  if (workPackages.length > 0) {
    return { status: 'ready' as const, metrics, progress, activeAgents, recentDelegations, pmPlan, nextAction: { label: 'Delegation erstellen', href: `/delegations?new=1&briefId=${brief.id}`, tone: 'success' as const } }
  }
  if (acceptedRequirements > 0 || milestones.length > 0) {
    return { status: 'planning' as const, metrics, progress, activeAgents, recentDelegations, pmPlan, nextAction: { label: 'Arbeitspakete planen', href: `/project-briefs/${brief.id}`, tone: 'warning' as const } }
  }
  return {
...
function buildProjectManagerPlan(input: {
  ...
  for (const delegation of input.linkedDelegations.filter(item => item.status === 'running').slice(0, 2)) {
    nextSteps.push({
      ...
      reason: 'Hier arbeitet bereits ein KI-Agent. Erst Fortschritt pruefen, bevor weitere parallele Arbeit startet.',
      href: `/delegations/${delegation.id}`,
      riskClass: delegation.contract.riskClass,
      canAutoStart: false,
      delegationId: delegation.id,
  ...
  for (const wp of input.workPackages.filter(item => !linkedWorkItemIds.has(item.id)).slice(0, 4)) {
    ...
    nextSteps.push({
      ...
      reason: isRisky
        ? 'Dieses Arbeitspaket braucht zuerst Klaerung, weil Risiko oder Blocker zu hoch sind.'
        : 'Dieses Arbeitspaket hat noch keine Delegation und ist ein sinnvoller naechster Schritt.',
  ...
  const summary = input.failedDelegations > 0
    ...
    : input.runningDelegations > 0
      ...
      : 'Sichere naechste Schritte sind bereit. Starte mit kleinen Delegations und pruefe danach den Fortschritt.'
