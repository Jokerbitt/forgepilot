...
const mockBrief = {
  ...
  rawIdea: 'Webhook raw idea',
  problemStatement: 'Webhook problem',
  targetAudience: 'Unbekannt',
  desiredOutcome: 'Zu definieren.',
  constraints: [],
  ...
  researchRunIds: [],
  researchBriefDraft: {
    title: 'Research Brief: Webhook-Idee',
    mode: 'quick' as const,
    privacyMode: 'local' as const,
...
describe('POST /api/webhooks/intake', () => {
  ...
  it('returns 200 with processed array for event: new-idea', async () => {
    ...
    const res = await POST(makeRequest({ event: 'new-idea', payload: { title: 'Test Idee' }, source: 'zapier' }))
  ...
  it('returns 400 when body is invalid JSON', async () => {
    const { POST } = await import('./route')
    const req = new Request('http://localhost/api/webhooks/intake', {
      method: 'POST',
  ...
  it('returns 200 for event: new-task', async () => {
    const { POST } = await import('./route')
    const res = await POST(makeRequest({ event: 'new-task', payload: { title: 'Auto Task' }, source: 'n8n' }))
    expect(res.status).toBe(200)
    const data = await res.json() as { received: boolean; processed: string[] }
    expect(data.received).toBe(true)
  ...
  it('returns 401 when WEBHOOK_SECRET is set but token is missing', async () => {
  ...
  it('returns 200 when WEBHOOK_SECRET is set and correct token provided', async () => {
    process.env.WEBHOOK_SECRET = 'my-secret'
    const { POST } = await import('./route')
