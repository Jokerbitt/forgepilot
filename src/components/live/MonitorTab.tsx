...
function getProviderColor(id: string): { dot: string; badge: string; border: string; accent: string } {
  if (id.includes('gemini') || id.includes('google'))
    return { dot: 'bg-blue-400', badge: 'bg-blue-500/10 text-blue-300 border-blue-500/25', border: 'border-blue-500/30', accent: 'text-blue-300' }
  if (id.includes('together') || id.includes('llama'))
    return { dot: 'bg-violet-400', badge: 'bg-violet-500/10 text-violet-300 border-violet-500/25', border: 'border-violet-500/30', accent: 'text-violet-300' }
  if (id.includes('ollama') || id.includes('local'))
    return { dot: 'bg-emerald-400', badge: 'bg-emerald-500/10 text-emerald-300 border-emerald-500/25', border: 'border-emerald-500/30', accent: 'text-emerald-300' }
  if (id.includes('openai') || id.includes('gpt'))
    return { dot: 'bg-teal-400', badge: 'bg-teal-500/10 text-teal-300 border-teal-500/25', border: 'border-teal-500/30', accent: 'text-teal-300' }
  return { dot: 'bg-slate-400', badge: 'bg-slate-500/10 text-slate-300 border-slate-500/25', border: 'border-slate-700', accent: 'text-slate-400' }
}
...
function agentStatusLabel(status: AgentActivity['status']): string {
...
function ActiveAgentCard({ agent }: { agent: AgentActivity }) {
  ...
  return (
    <div className={cx('rounded-lg border px-4 py-3', isRunning ? 'border-emerald-500/30 bg-emerald-950/20' : 'border-slate-800 bg-slate-900/40')}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {agent.task && <p className="mt-1 text-xs italic text-slate-500">{agent.task}</p>}
          <div className="mt-2 flex flex-wrap items-center gap-3 text-xs text-slate-500">
            ...
            {isRunning && <><span>·</span><span className="tabular-nums">{Math.round(elapsed / 1000)}s</span></>}
          </div>
        </div>
      </div>
      {isRunning && (
        <div className="mt-3">
          <div className="h-1 w-full overflow-hidden rounded-full bg-slate-800">
            <div className="h-1 rounded-full bg-emerald-500" style={{ width: '60%', animation: 'pulse 1.5s ease-in-out infinite' }} />
          </div>
        </div>
      )}
    </div>
...
function ProviderCard({ stats }: { stats: ProviderStats }) {
  ...
  const quotaPct = hasQuota ? Math.round(((stats.freeQuotaUsed ?? 0) / (stats.freeQuotaLimit ?? 1)) * 100) : 0
  return (
    <div className={cx('rounded-xl border p-4', colors.border, 'bg-white/[0.02]')}>
      ...
      <div className="mb-3">
        <span className="rounded border border-slate-700 bg-slate-800/60 px-1.5 py-0.5 text-[10px] font-mono text-slate-400">{stats.model}</span>
      </div>
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="rounded bg-slate-900/60 px-2 py-1.5 text-center">
          <p className={cx('text-base font-bold tabular-nums', stats.callsToday > 0 ? colors.accent : 'text-slate-500')}>{stats.callsToday}</p>
          <p className="text-[9px] uppercase tracking-wide text-slate-600">Calls h.</p>
        </div>
        <div className="rounded bg-slate-900/60 px-2 py-1.5 text-center">
          <p className={cx('text-base font-bold tabular-nums', stats.tokensToday > 0 ? 'text-slate-200' : 'text-slate-500')}>{formatTokens(stats.tokensToday)}</p>
          <p className="text-[9px] uppercase tracking-wide text-slate-600">Tokens h.</p>
        </div>
        <div className="rounded bg-slate-900/60 px-2 py-1.5 text-center">
          <p className={cx('text-base font-bold tabular-nums', stats.costTodayUsd > 0 ? 'text-cyan-400' : 'text-slate-500')}>{formatCost(stats.costTodayUsd)}</p>
      ...
      {hasQuota && (
        <div>
          <div className="mb-1 flex items-center justify-between text-[10px]">
            ...
            <span className="font-mono text-slate-400 tabular-nums">{stats.freeQuotaUsed} / {stats.freeQuotaLimit} ({quotaPct}%)</span>
          </div>
          <div className="h-1.5 w-full overflow-hidden rounded-full bg-slate-800">
            <div className={cx('h-full rounded-full transition-all', getQuotaColor(stats.freeQuotaUsed ?? 0, stats.freeQuotaLimit ?? 1))} style={{ width: `${Math.min(quotaPct, 100)}%` }} />
          </div>
        </div>
      )}
      {stats.errorRate > 0 && <div className="mt-2 text-[10px] text-rose-400">Fehlerrate: {Math.round(stats.errorRate * 100)}%</div>}
    </div>
  )
}
...
function GeminiQuotaWidget({ quota }: { quota: GeminiQuotaStatus | null }) {
  ...
  const barColor = percentage >= 95 ? 'bg-rose-500' : percentage >= 80 ? 'bg-amber-400' : 'bg-emerald-500'
  const textColor = percentage >= 95 ? 'text-rose-300' : percentage >= 80 ? 'text-amber-300' : 'text-emerald-300'
  ...
  return (
    <Panel className="p-5">
      <div className="mb-3 flex items-center justify-between">
        ...
        <span className={cx('text-xs font-semibold tabular-nums', textColor)}>{percentage}%</span>
      </div>
      ...
      <div className="mb-3 h-2 w-full overflow-hidden rounded-full bg-slate-800">
        <div className={cx('h-full rounded-full transition-all', barColor)} style={{ width: `${Math.min(percentage, 100)}%` }} />
      </div>
      <p className="text-xs text-slate-500">{resetLabel}</p>
      {!quota && <p className="mt-1 text-[10px] text-amber-500">Demo-Daten — /api/monitor/quota nicht aktiv</p>}
    </Panel>
  )
}
...
export function MonitorTab() {
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => { void fetchSnapshot(); void fetchGeminiQuota() }, 30000)
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <p className="text-xs text-slate-500">
          {useMock && <span className="text-amber-400">Demo-Daten — /api/monitor nicht aktiv · </span>}
          Aktualisiert {formatRelativeTime(lastRefresh.toISOString())}
        </p>
        <button onClick={() => void fetchSnapshot()} className={buttonClassName('ghost', 'text-xs')}>↻ Refresh</button>
      </div>
      ...
      <Panel className="p-5">
        <div className="mb-4 flex items-center justify-between">
          ...
          {hasActive && <Badge tone="success">{snapshot.activeAgents.length} aktiv</Badge>}
        </div>
        {!hasActive ? (
          ...
          <div className="space-y-3">
            {snapshot.activeAgents.map(agent => <ActiveAgentCard key={agent.id} agent={agent} />)}
          </div>
        )}
      </Panel>
      ...
      {snapshot.providerStats.length > 0 && (
        <div>
          <div className="mb-3 flex items-center gap-2">
            ...
            <span className="rounded-full bg-slate-800 px-2 py-0.5 text-xs text-slate-400">{snapshot.providerStats.length}</span>
          </div>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {snapshot.providerStats.map(provider => <ProviderCard key={provider.providerId} stats={provider} />)}
          </div>
        </div>
      )}
        ...
        <Panel className="p-5">
          ...
          <div className="space-y-3">
            {snapshot.recommendations.map((rec, i) => (
              <div key={i} className={cx('flex items-start gap-3 rounded-lg border px-4 py-3', severityBg(rec.severity))}>
                ...
                {rec.action && <button className={buttonClassName('ghost', 'shrink-0 text-xs')}>{rec.action}</button>}
              </div>
            ))}
          </div>
        </Panel>
              ...
              <tbody className="divide-y divide-slate-800/50">
                {recentSlice.map(agent => (
                  <tr key={agent.id} className="hover:bg-slate-800/20">
                    <td className="py-2.5 pr-4 text-slate-500 whitespace-nowrap">{formatRelativeTime(agent.startedAt)}</td>
                    <td className="py-2.5 pr-4 max-w-[180px]">
                      <p className="truncate font-medium text-slate-200">{agent.name}</p>
                      {agent.task && <p className="truncate text-slate-600 italic">{agent.task}</p>}
                    </td>
                    <td className="py-2.5 pr-4 text-slate-400 hidden sm:table-cell whitespace-nowrap">{agent.model}</td>
                    <td className="py-2.5 pr-4 text-right font-mono text-slate-400 tabular-nums">{formatTokens(agent.totalTokens)}</td>
                    <td className="py-2.5 pr-4 text-right font-mono text-slate-400 tabular-nums">{formatCost(agent.costUsd)}</td>
                    <td className="py-2.5 pr-4 text-right text-slate-500 hidden md:table-cell">{formatDuration(agent.durationMs)}</td>
                    <td className="py-2.5 text-center"><RunStatusBadge status={agent.status} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <Panel className="p-5">
        <div className="mb-4">
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Übersicht</p>
          <h2 className="mt-1 text-lg font-semibold text-white">Heute & Diesen Monat</h2>
        </div>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <SummaryMetric label="Calls heute" value={String(snapshot.totals.callsToday)} sub="API-Aufrufe (24h)" color={snapshot.totals.callsToday > 0 ? 'text-violet-400' : 'text-slate-500'} />
          <SummaryMetric label="Tokens heute" value={formatTokens(snapshot.totals.tokensToday)} sub="Input + Output" color={snapshot.totals.tokensToday > 0 ? 'text-blue-400' : 'text-slate-500'} />
          <SummaryMetric label="Kosten Monat" value={formatCost(snapshot.totals.costThisMonthUsd)} sub="laufender Monat" color="text-cyan-400" />
          <SummaryMetric label="Erfolgsrate" value={`${Math.round(snapshot.totals.successRate * 100)}%`} sub={`Ø ${Math.round(snapshot.totals.avgResponseMs)}ms Latenz`} color={snapshot.totals.successRate >= 0.9 ? 'text-emerald-400' : snapshot.totals.successRate >= 0.7 ? 'text-amber-400' : 'text-rose-400'} />
        </div>
      </Panel>
    </div>
  )
