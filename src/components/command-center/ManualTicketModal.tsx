...
export function ManualTicketModal({ isOpen, onClose }: ManualTicketModalProps) {
    ...
    try {
      await fetch('/api/magic-create', {
        ...
        body: JSON.stringify({
          ...
          title,
          description,
          projectId: projectId.trim() || 'LOCAL_IDEAS',
      ...
      <div className="bg-gray-950 border border-gray-800 rounded-xl shadow-2xl w-full max-w-2xl flex flex-col overflow-hidden max-h-[90vh]">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input 
                ...
                placeholder="LOCAL_IDEAS"
                className="w-full bg-gray-900 border border-gray-700 rounded-md px-3 py-2 text-white"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-400 mb-1">Meilenstein (Optional)</label>
              <input 
                type="text" 
                value={milestone} 
                onChange={e => setMilestone(e.target.value)}
                placeholder="z.B. M5, Q3-Release"
                className="w-full bg-gray-900 border border-gray-700 rounded-md px-3 py-2 text-white"
              />
            </div>
          </div>
        ...
        <footer className="px-6 py-4 border-t border-gray-800 bg-gray-900 flex justify-end space-x-3">
          ...
          <button 
            ...
            className="px-4 py-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white rounded font-bold transition-colors"
          >
