...
const SHORTCUTS: Array<{ key: string; desc: string; section: string }> = [
  ...
  { section: 'Navigation', key: 'G I', desc: 'Inbox' },
  { section: 'Navigation', key: 'G B', desc: 'Agent Board' },
      ...
      >
        ...
        <div className="p-4 space-y-4">
          {sections.map(section => (
            <div key={section}>
              <p className="mb-2 text-[10px] font-semibold uppercase tracking-wide text-slate-600">{section}</p>
        ...
        <div className="border-t border-slate-800 px-4 py-2">
          <p className="text-[10px] text-slate-600">Drücke <kbd className="font-mono">Esc</kbd> zum Schließen</p>
        </div>
      </div>
    </div>
  )
