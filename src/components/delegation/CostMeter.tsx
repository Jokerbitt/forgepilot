...
export function CostMeter({ actualCostUsd, estimateCostUsd, maxBudgetUsd }: Props) {
  const ratio = maxBudgetUsd != null && estimateCostUsd != null
    ? computeCostRatio(actualCostUsd, estimateCostUsd, maxBudgetUsd)
    : 0
    ...
    <div className="flex flex-col gap-1">
      <div className="flex items-center justify-between">
        <span className={`text-xs font-bold font-mono ${isActual ? 'text-yellow-400' : 'text-gray-400'}`}>
          ...
          {!isActual && displayCost != null && <span className="text-gray-600 text-[10px] ml-1">(est.)</span>}
        </span>
        <span className="text-[10px] text-gray-600 font-mono">{maxBudgetUsd != null ? `${pct}%` : ''}</span>
      </div>
