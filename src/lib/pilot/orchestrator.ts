...
export async function runPilot(input: PilotInput): Promise<PilotRunResult> {
  ...
  // Step 4: Agent run create — generate AI execution plan + create run record
  const runTraceStep = await runStep('agent-run-create', async () => {
    const { createRun, updateRun } = await import('@/lib/agent-runs/store')
    ...
    // Attempt real AI plan generation; fall back to static plan if not configured
    const { generateExecutionPlan, buildFallbackPlan } = await import('@/lib/pilot/ai-plan')
    let plan
    let aiUsed = false
    try {
      ...
      // Fall back gracefully for any AI error (no key, no credits, network, etc.)
      plan = buildFallbackPlan({ title: input.title, goal: input.goal, riskClass, privacyMode })
    }
    ...
    const resultSummary = [
      ...
      '',
      aiUsed ? `*Generated by ${plan.model} (${plan.provider})*` : '*Fallback plan (AI not configured)*',
    ].join('\n')

    updateRun(run.id, { resultSummary })
    return { runId: run.id, status: run.status, aiUsed, complexity: plan.estimatedComplexity }
  })
  steps.push(runTraceStep)

  // Step 5: Writeback summary — persist run summary + create memory cards
  const writebackStep = await runStep('writeback', async () => {
    ...
    return { markdownLength: markdown.length, hasContent: markdown.length > 0 }
  })
